<%@ page contentType="text/html;charset=UTF-8" language="java" %>

<%@include file="../appParts/header.jsp"%>

<section class="dashboard-section">
    <div class="row dashboard-nowrap">
<%@include file="../appParts/menu.jsp" %>

            <div class="m-4 p-3 width-medium">
                <div class="dashboard-content border-dashed p-3 m-4 view-height">
                    <form:form method="POST" modelAttribute="plan">
                        <div class="row border-bottom border-3 p-1 m-1">
                            <div class="col noPadding">
                                <h3 class="color-header text-uppercase">NOWY PLAN</h3>
                            </div>
                            <div class="col d-flex justify-content-end mb-2 noPadding">
                                <form:button type="submit" class="btn btn-color rounded-0 pt-0 pb-0 pr-4 pl-4">Zapisz</form:button>
                            </div>
                        </div>

                        <div class="schedules-content">

                            <div class="form-group row">
                                <label for="planName" class="col-sm-2 label-size col-form-label">
                                    Nazwa planu
                                </label>
                                <div class="col-sm-10">
                                    <form:input class="form-control" name="planName" id="planName" placeholder="Nazwa planu" path="name"/>
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="planDescription" class="col-sm-2 label-size col-form-label">
                                    Opis planu
                                </label>
                                <div class="col-sm-10">
                                <form:textarea class="form-control" rows="5" name="planDescription" id="planDescription"
                                          placeholder="Opis planu" path="description"/>
                                </div>
                            </div>

                        </div>
                    </form:form>
                </div>
            </div>
        </div>
    </section>

<%@include file="../appParts/footer.jsp"%>